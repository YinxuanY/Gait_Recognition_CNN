...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # extract fine-grained feature
        branch_channels = 1
        self.branch1 = SetBlock(BasicConv2d(branch_channels, 32, 1))
        self.branch2 = SetBlock(BasicConv2d(branch_channels, 32, 3, padding=1))
        self.branch3 = SetBlock(BasicConv2d(branch_channels, 32, 5, padding=2))

        _set_in_channels = 1
        _set_channels = [32*3, 128, 256]
        ...
        self.part_feat = nn.ModuleList([BasicConv2d(_set_channels[0], _set_channels[1], 3, padding=1) \
                                                        for _ in range(2)])

        # multi-resolution fuse layer
        self.c_3x3 = BasicConv2d(_set_channels[1], _set_channels[1], 3, stride=2, padding=1)
        self.c_1x1_1 = BasicConv2d(_set_channels[1], _gl_channels[1], 1)
        self.c_1x1_2 = BasicConv2d(_gl_channels[1], _set_channels[1], 1)

        self.bin_num = [1, 2, 4, 8, 16]
        self.fc_bin = nn.ParameterList([
            nn.Parameter(
                nn.init.xavier_uniform_(
                    torch.zeros(sum(self.bin_num) * 3, 256, hidden_dim)))])
    ...
    def forward(self, silho, batch_frame=None):
        ...
        feat_list = []
        for i in range(2):
            feat = self.frame_max(x)[0][:, :, int(i*h/2):int((i+1)*h/2), :]
            feat_list.append(self.part_feat[i](feat))
        feat = torch.cat(feat_list, -2)

        # using_c_3x3
        conv3x3 = self.c_3x3(feat)
        ...
        # using_c_1x1_1
        conv1x1 = self.c_1x1_1(self.frame_max(x)[0])
        ...
        # using_c_1x1_2, fusing feature
        conv1x1 = gl + conv1x1
        fuse_feat = torch.cat((conv3x3, self.c_1x1_2(conv1x1)), 1)
        ...
        for num_bin in self.bin_num:
            ...
            z = z.mean(3) + z.max(3)[0]
            feature.append(z)
            z = fuse_feat.view(n, c, num_bin, -1)
            z = z.mean(3) + z.max(3)[0]
            feature.append(z)
